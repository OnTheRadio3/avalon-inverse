...
func _process_state(delta:float) -> void:
	...
	if movement_input != Vector3.ZERO:
		p_parent.m_x_speed = p_parent.m_max_speed
	else:
		p_parent.m_x_speed -= (p_parent.m_acceleration - p_parent.m_x_speed * p_parent.m_damping) * delta
	
	p_parent.m_x_speed = clampf(p_parent.m_x_speed, 0, p_parent.m_max_speed)
	print(p_parent.m_x_speed)
